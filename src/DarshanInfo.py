        ...
        if os.path.isdir(self.path):
            ...
            for it, filename in enumerate(filenames):
                if filename.split('.')[-1] != 'darshan':
                    print("The file {} is not a darshan file".format(file))
                
                f = DarshanFile(self.path + '/' + filename)
                self.files.append(f)
                self.nprocs += f.nprocs
                self.start = f.report.start_time if not self.start or self.start > f.report.start_time else self.start
                self.end = f.report.end_time if not self.end or self.end < f.report.end_time else self.end
