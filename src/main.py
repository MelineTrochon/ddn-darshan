...
def usage():
    ...
    print(
        "nb_rank_file : Shows the number of rank that access the same file each time step"
    )
...
def main():
    ...
    while info.i < len(argv):
        ...
        if argv[info.i] == "aggregate_info":
            start_t = time.time()
            info.aggregate_info()
            print("aggregate_info took %.3f seconds" % (time.time() - start_t))
            continue
