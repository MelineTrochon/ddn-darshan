...
class FileNbRankPerSec:
    ...
    def get_data(self, info):
        ...
        self.rank_mat = [SparseMatrix(info, self.value[i], self.x[i], self.y[i], shape=self.shape, dtype='int32') for i in range(info.nprocs)]
        # print(info.nprocs)
        # print(self.rank_mat[0].mat.data)
        for i in range(info.nprocs):
            # print(self.rank_mat[i].mat.data)
            # print(max(self.rank_mat[i].mat.data))
            # print(self.rank_mat[i].mat.toarray())
            self.rank_mat[i].mat.data = np.ones(len(self.rank_mat[i].mat.data), dtype='int32')
    ...
    def to_heatmap(self, info):
        ...
        fig.suptitle('Heatmap of the number of rank {}ing on the same file'.format(info.op))
        extent = [0, info.len_dxt_posix, 0, info.nbins]
        # print(self.mat.data)
        # vmax = info.nprocs
        vmax = max(self.mat.data)
        norm = info.norm
        # self.mat.plot_heatmap(axs, str(info.op))

        def plot_heatmap(self, ax, title):
            ax.imshow(self.mat.toarray(), aspect='auto', cmap='Reds', interpolation='nearest', origin='lower', extent=extent, norm=norm, vmax=vmax)
            ax.set_title(title)
            ax.grid(True)
