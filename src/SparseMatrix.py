...
"""
This class is a sub-class the creates Sparse Matrices from data that can be plotted as heatmaps.
"""
...
class SparseMatrix:
    ...
    def plot_heatmap(self, ax, title):
        ax.imshow(self.mat.toarray(), aspect='auto', cmap='Reds', interpolation='nearest', origin='lower', extent=self.extent, norm=self.norm, vmax=self.vmax)
        ax.set_title(title)
        ax.grid(True)
